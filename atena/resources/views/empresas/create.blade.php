...
<div class="max-w-xl mx-auto mt-8 px-4">
    <h1 class="text-2xl font-bold mb-6 text-red-400">Nueva Empresa</h1>
    <form method="POST" action="{{ route('empresas.store') }}" class="bg-gray-800 p-6 rounded shadow space-y-4">
        @csrf

        @foreach (['nombre','nif','email','telefono','direccion','ciudad','ceo'] as $campo)
        <div>
            <label class="block mb-1 capitalize" for="{{ $campo }}">{{ $campo }}:</label>
            <input type="{{ $campo === 'email' ? 'email' : 'text' }}" name="{{ $campo }}" id="{{ $campo }}"
